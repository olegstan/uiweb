...
class PaymentMethodsControllerAdmin extends Controller
{
    ...
    function fetch()
    {
        ...
        if (trim($this->param_url, '/') == "save_positions")
        {
            ...
            foreach($menu as $parent_id=>$items)
                foreach($items as $position=>$id)
                    $this->payment->update_payment_method($id, array('position'=>$position));
        ...
        $this->design->assign('edit_module', $this->furl->get_module_by_name('PaymentMethodControllerAdmin'));
        return $this->design->fetch($this->design->getTemplateDir('admin').'payment-methods.tpl');
    }
}