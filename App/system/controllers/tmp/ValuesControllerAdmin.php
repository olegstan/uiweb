...
class ValuesControllerAdmin extends Controller
{
    ...
    function fetch()
    {
        ...
        foreach($this->params_arr as $p=>$v)
        {
            switch ($p)
            {
                case "save_positions":
                    ...
                    foreach($menu as $parent_id=>$items)
                        foreach($items as $position=>$id)
                            $this->tags->update_tag($id, array('position'=>$position));
                ...
                case "group_id":
                    ...
                    {
                        $tmp_group = $this->tags->get_taggroup(intval($v));
                        if ($tmp_group->is_auto)
                            unset($this->params_arr[$p]);
                        else
                            $tags_filter[$p] = intval($v);
                    }
                    else
                        unset($this->params_arr[$p]);
                    break;
                case "keyword":
                    if (!empty($this->params_arr[$p]))
                    {
                        $tags_filter[$p] = $this->params_arr[$p];
                        $this->design->assign('keyword', $tags_filter['keyword']);
                    }
                    else
                        unset($this->params_arr[$p]);
                    break;
                ...
                case "sort_type":
                    if (!empty($this->params_arr[$p]))
                    {
                        if (!array_key_exists('sort_type', $tags_filter))
                            $tags_filter[$p] = $this->params_arr[$p];
                    }
                    else
                        unset($this->params_arr[$p]);
        ...
        if (!array_key_exists('sort', $this->params_arr))
        {
            ...
            $tags_filter['sort'] = 'position';
            $tags_filter['sort_type'] = 'asc';
        }
        else
        ...
        $all_tags_count = $this->tags->count_tags(array('is_auto'=>0));
        $tags_count = $this->tags->count_tags($tags_filter);
        ...
        $this->design->assign('all_tags_count', $all_tags_count);
        $this->design->assign('tags_count', $tags_count);
        $pages_num = ceil($tags_count/$items_per_page);
        $this->design->assign('total_pages_num', $pages_num);

        $tags_filter['page'] = $current_page;
        $tags_filter['limit'] = $items_per_page;

        $this->design->assign('tags', $this->tags->get_tags($tags_filter));

        $tags_groups = $this->tags->get_taggroups(array('is_auto'=>0));
        foreach($tags_groups as &$group)
            $group->tags_count = $this->tags->count_tags(array('group_id'=>$group->id, 'is_auto'=>0));

        $this->design->assign('tags_groups', $tags_groups);
        $this->design->assign('params_arr', $this->params_arr);
        $this->design->assign('edit_module', $this->furl->get_module_by_name('ValueControllerAdmin'));
        return $this->design->fetch($this->design->getTemplateDir('admin').'values.tpl');
    }
}