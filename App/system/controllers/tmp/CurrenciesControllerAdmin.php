...
class CurrenciesControllerAdmin extends Controller
{
    ...
    function fetch()
    {
        ...
        if (trim($this->param_url, '/') == "save_positions")
        {
            ...
            foreach($menu as $parent_id=>$items)
                foreach($items as $position=>$id)
                    $this->currencies->update_currency($id, array('position'=>$position));
        ...
        $this->design->assign('currencies', $this->currencies->get_currencies());
        $this->design->assign('edit_module', $this->furl->get_module_by_name('CurrencyControllerAdmin'));
        return $this->design->fetch($this->design->getTemplateDir('admin').'currencies.tpl');
    }
}