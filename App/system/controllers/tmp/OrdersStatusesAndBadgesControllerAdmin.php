...
class OrdersStatusesAndBadgesControllerAdmin extends Controller
{
    ...
    function fetch()
    {
        ...
        foreach($this->params_arr as $p=>$v)
        {
            switch ($p)
            {
                case "save_positions":
                    ...
                    foreach($menu as $parent_id=>$items)
                        foreach($items as $position=>$id)
                            $this->orders->update_status($id, array('position'=>$position));
                ...
                case "sort":
                    if (!empty($this->params_arr[$p]))
                        $statuses_filter[$p] = $this->params_arr[$p];
                    else
                        unset($this->params_arr[$p]);
                    break;
                case "sort_type":
                    if (!empty($this->params_arr[$p]))
                    {
                        if (!array_key_exists('sort_type', $statuses_filter))
                            $statuses_filter[$p] = $this->params_arr[$p];
                    }
                    else
                        unset($this->params_arr[$p]);
        ...
        $statuses_count = $this->orders->count_statuses($statuses_filter);
        ...
        $this->design->assign('statuses_count', $statuses_count);
        $pages_num = ceil($statuses_count/$items_per_page);
        $this->design->assign('total_pages_num', $pages_num);

        $statuses_filter['page'] = $current_page;
        $statuses_filter['limit'] = $items_per_page;

        $this->design->assign('orders_statuses', $this->orders->get_statuses($statuses_filter));
        $this->design->assign('params_arr', $this->params_arr);
        $this->design->assign('edit_module', $this->furl->get_module_by_name('OrdersStatusControllerAdmin'));
        return $this->design->fetch($this->design->getTemplateDir('admin').'orders-statuses.tpl');
    }
}