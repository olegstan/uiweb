...
class FiltersForCategoryControllerAdmin extends Controller
{
    ...
    function fetch()
    {
        ...
        foreach($this->params_arr as $p=>$v)
        {
            switch ($p)
            {
                case "save_positions":
                    ...
                    foreach($menu as $parent_id=>$items)
                        foreach($items as $position=>$id)
                            $this->tags->update_tags_set($id, array('position'=>$position));
                ...
                case "keyword":
                    ...
                    {
                        $sets_filter[$p] = $this->params_arr[$p];
                        $this->design->assign('keyword', $sets_filter['keyword']);
                    }
                    else
                ...
                case "sort_type":
                    if (!empty($this->params_arr[$p]))
                    {
                        if (!array_key_exists('sort_type', $sets_filter))
                            $sets_filter[$p] = $this->params_arr[$p];
                    }
                    else
                        unset($this->params_arr[$p]);
        ...
        if (!array_key_exists('sort', $this->params_arr))
        {
            ...
            $sets_filter['sort'] = 'name';
            $sets_filter['sort_type'] = 'asc';
        }
        else
        ...
        $sets_count = $this->tags->count_tags_sets($sets_filter);

        // Постраничная навигация
        if (array_key_exists('page', $this->params_arr) && $this->params_arr['page'] == 'all')
            $items_per_page = $sets_count;
        else
            $items_per_page = $this->settings->products_num_admin;

        $this->design->assign('sets_count', $sets_count);
        $pages_num = ceil($sets_count/$items_per_page);
        $this->design->assign('total_pages_num', $pages_num);

        $sets_filter['page'] = $current_page;
        $sets_filter['limit'] = $items_per_page;

        $this->design->assign('tags_sets', $this->tags->get_tags_sets($sets_filter));
        $this->design->assign('params_arr', $this->params_arr);
        $this->design->assign('edit_module', $this->furl->get_module_by_name('FilterForCategoryControllerAdmin'));
        return $this->design->fetch($this->design->getTemplateDir('admin').'filters-for-category.tpl');
    }
}