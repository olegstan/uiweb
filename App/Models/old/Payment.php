...
class Payment extends LayerModel
{
    public function get_payment_methods($filter = array())
    {
        $delivery_filter = '';
        if(!empty($filter['delivery_id']))
            $delivery_filter = $this->db->placehold('AND id in (SELECT payment_method_id FROM __deliveries_payment dp WHERE dp.delivery_id=?)', intval($filter['delivery_id']));
        ...
        $query = "SELECT *
                    FROM __payment_methods WHERE 1 $delivery_filter $is_enabled_filter ORDER BY position";
    ...
    {
        $query = $this->db->placehold("SELECT delivery_id FROM __deliveries_payment WHERE payment_method_id=?", intval($id));
        $this->db->query($query);
    ...
    {
        $query = $this->db->placehold("DELETE FROM __deliveries_payment WHERE payment_method_id=?", intval($id));
        $this->db->query($query);
        if(is_array($deliveries_ids))
        foreach($deliveries_ids as $d_id)
            $this->db->query("INSERT INTO __deliveries_payment SET payment_method_id=?, delivery_id=?", $id, $d_id);
    }
    ...
    public function delete_payment_method($id)
    {
        if(!empty($id))
        {
            ...
            foreach($images as $i)
                $this->image->delete_image('payment', $id, $i->id);
    ...
    public function get_delivery_methods($id)
    {
        $query = $this->db->placehold("SELECT delivery_id FROM __deliveries_payment WHERE payment_method_id=?", intval($id));
        $this->db->query($query);
        return $this->db->results('delivery_id');
    ...
    public function update_delivery_methods($id, $delivery_methods_ids)
    {
        $query = $this->db->placehold("DELETE FROM __deliveries_payment WHERE payment_method_id=?", intval($id));
        $this->db->query($query);
        if(is_array($delivery_methods_ids))
            foreach($delivery_methods_ids as $p_id)
                $this->db->query("INSERT INTO __deliveries_payment SET delivery_id=?, payment_method_id=?", $p_id, $id);
    }
}